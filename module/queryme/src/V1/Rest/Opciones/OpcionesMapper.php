...
class OpcionesMapper
{
	...
	public function getData($oppr){
		...
		foreach($oppr as $val){
			...
			if ($val == end($oppr)){
				$data .=  "{ value".$oppr['0']['idpregunta'];
					$data .= ": ".$val['idopcion'].", display";
					$data .= $oppr['0']['idpregunta'];
					$data .= ": '".$val['valorOpcion'];
					$data .= "', habilita".$oppr['0']['idpregunta'];
					$data .= ": '".$val['habilita_idpanel']."'}";
				}else{
					$data.= "{ value".$oppr['0']['idpregunta'];
						$data.= ": ".$val['idopcion'].", display";
						$data.= $oppr['0']['idpregunta'].": '";
